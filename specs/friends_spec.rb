...
class TestFriends < MiniTest::Test
  ...
  # (e.g. the function likes_to_eat(@person2, "bread") should return true, likes_to_eat(@person3, "spinach") should return false)
    def test_likes_to_eat_p_2
      result = likes_to_eat(@person2, 'bread')
      assert_equal(true, result)
    end

    def test_likes_to_eat_p_3
  ...
  def test_loan_money
    loan_money(@person5, @person1, 5)
    result1 = @person5[:monies]
    result2 = @person1[:monies]
    assert_equal(result1, 95)
    assert_equal(result2, 6)
  end

  # 8. Find the set of everyone's favourite food joined together
  # (hint: concatenate the favourites/snack arrays together)
    def test_everyones_snacks
      result = everyones_snacks(@people)
